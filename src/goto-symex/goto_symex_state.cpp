  ...
  levelt level)
{
  ...
  else
  {
    ...
    // do this recursively
    Forall_operands(it, expr)rename(*it, ns, manager, level);
  ...
  guard_managert &manager)
{
  ...
  if(atomic_section_id!=0)
  {
    ...
    {
      guardt no_write_guard(guard_manager);
  ...
  guard_managert &manager)
{
  ...
  else
  {
    ...
    else
    {
      ...
      Forall_operands(it, expr)rename_address(*it, ns, level, manager);
    }
  }
}
          ...
          type.id()==ID_class)
  {
    ...
    for(struct_union_typet::componentst::iterator
      ...
      if(it->type().id()==ID_array)
        rename(to_array_type(it->type()).size(), ns, manager, level);
      else if(it->type().id()!=ID_pointer)
        rename(it->type(), irep_idt(), ns, manager, level);
  }
