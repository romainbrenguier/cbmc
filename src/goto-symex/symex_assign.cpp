  ...
  symex_targett &target)
{
  ...
  {
    symex_assign_symbol(
      state, to_ssa_expr(lhs), full_lhs, rhs, guard, assignment_type, ns,
      symex_config, target);
  }
  else if(lhs.id()==ID_index)
    symex_assign_array(
      state, to_index_expr(lhs), full_lhs, rhs, guard, assignment_type, ns, symex_config, target);
  else if(lhs.id()==ID_member)
  {
    ...
    if(type.id() == ID_struct || type.id() == ID_struct_tag)
      symex_assign_struct_member(
        state, to_member_expr(lhs), full_lhs, rhs, guard, assignment_type, ns, symex_config, target);
    ...
    symex_assign_if(
      state, to_if_expr(lhs), full_lhs, rhs, guard, assignment_type, ns, symex_config, target);
  else if(lhs.id()==ID_typecast)
    symex_assign_typecast(
      state, to_typecast_expr(lhs), full_lhs, rhs, guard, assignment_type, ns, symex_config, target);
          ...
          lhs.id()==ID_byte_extract_big_endian)
  {
    symex_assign_byte_extract(
      state, to_byte_extract_expr(lhs), full_lhs, rhs, guard, assignment_type, ns, symex_config, target);
  }
  else if(lhs.id() == ID_complex_real)
  {
    ...
    symex_assign_rec(
      state, complex_real_expr.op(), full_lhs, new_rhs, guard, assignment_type, ns, symex_config, target);
  }
  ...
  for(const auto &comp_rhs : make_range(components).zip(rhs.operands()))
  {
    ...
    if(comp_rhs.second.id() == ID_struct)
    {
      symex_assign_from_struct(
        state, to_ssa_expr(lhs_field),
        full_lhs, to_struct_expr(comp_rhs.second), guard, assignment_type,
        ns, symex_config, target);
    }
    else
    {
  ...
  symex_targett &target)
{
  ...
  {
    symex_assign_from_struct(
      state, lhs, full_lhs, to_struct_expr(rhs), guard, assignment_type, ns,
      symex_config, target);
  }

  assign_non_struct_symbol(
    state, lhs, full_lhs, rhs, guard, assignment_type, ns, symex_config,
    target);
}
