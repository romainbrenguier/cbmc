...
void goto_symext::symex_decl(statet &state, const symbol_exprt &expr)
{
  ...
  ssa_exprt ssa(expr);
  state.rename(ssa, ns, guard_manager, goto_symex_statet::L1);
  const irep_idt &l1_identifier=ssa.get_identifier();

  // rename type to L2
  state.rename(ssa.type(), l1_identifier, ns, guard_manager);
  ssa.update_type();
  ...
  if(ns.follow(expr.type()).id()==ID_pointer)
  {
    ...
    state.rename(rhs, ns, guard_manager, goto_symex_statet::L1);
  ...
  state.rename(ssa, ns, guard_manager);
