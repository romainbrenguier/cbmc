...
{
  get_objects_rec(mode, expr.op(), range_start, size, guard_manager);
}
  ...
  const range_spect &size)
{
  ...
  get_objects_rec(mode, op, range_start + offset, size, guard_manager);
}
  ...
  const range_spect &size)
{
  if(if_expr.cond().is_false())
    get_objects_rec(
      mode, if_expr.false_case(), range_start, size, guard_manager);
  else if(if_expr.cond().is_true())
    get_objects_rec(
      mode, if_expr.true_case(), range_start, size, guard_manager);
  else
  {
  ...
  const range_spect &size)
{
  ...
  if(range_start==-1 || to_integer(simp_offset, index))
    get_objects_rec(mode, be.op(), -1, size, guard_manager);
  else
  {
    index*=8;
    ...
    get_objects_rec(mode, be.op(), offset, size, guard_manager);
  }
}
  ...
  const range_spect &size)
{
  ...
  {
    get_objects_rec(mode, shift.op(), -1, -1, guard_manager);
    get_objects_rec(mode, shift.distance(), -1, -1, guard_manager);
  }
  else
  {
    ...
    if(shift.id()==ID_ashr || shift.id()==ID_lshr)
    {
      ...
      if(sh_range_start>=0 && sh_range_start<src_size)
        get_objects_rec(
          mode, shift.op(), sh_range_start, sh_size, guard_manager);
    }
    else
    {
      ...
      get_objects_rec(mode, shift.op(), range_start, sh_size, guard_manager);
    }
  }
}
  ...
  const range_spect &size)
{
  ...
  {
    get_objects_rec(mode, expr.struct_op(), range_start, size, guard_manager);
    return;
  }
  ...
  get_objects_rec(mode, expr.struct_op(), offset, size, guard_manager);
}
  ...
  const range_spect &size)
{
  ...
  if(range_start == -1 || sub_size == -1 || !index.has_value())
    get_objects_rec(mode, expr.array(), -1, size, guard_manager);
  else
    get_objects_rec(
      mode,
      expr.array(),
  ...
  const range_spect &size)
{
  ...
  else
  {
    forall_operands(it, expr)
      get_objects_rec(mode, *it, 0, -1, guard_manager);
  ...
  forall_operands(it, expr)
  {
    if(full_r_s<=offset+sub_size && full_r_e>offset)
    {
      ...
      get_objects_rec(mode, *it, cur_r_s, cur_r_e - cur_r_s, guard_manager);
    }
  ...
  const range_spect &size)
{
  ...
  forall_operands(it, expr)
  {
    ...
    if(offset==-1)
    {
      get_objects_rec(mode, *it, 0, sub_size, guard_manager);
    }
    else if(sub_size==-1)
    {
      if(full_r_e==-1 || full_r_e>offset)
      {
        ...
        get_objects_rec(mode, *it, cur_r_s, -1, guard_manager);
      }
    ...
    else if(full_r_e==-1)
    {
      if(full_r_s<=offset+sub_size)
      {
        ...
        get_objects_rec(mode, *it, cur_r_s, sub_size - cur_r_s, guard_manager);
      }
  ...
  const range_spect &size)
{
  ...
  get_objects_rec(mode, op, range_start, new_size, guard_manager);
}
  ...
  guard_managert &guard_manager)
{
  if(expr.id() == ID_complex_real)
    ...
    get_objects_dereference(
      mode, to_dereference_expr(expr), range_start, size, guard_manager);
...
void rw_range_sett::get_objects_rec(get_modet mode, const exprt &expr)
{
  ...
  get_objects_rec(mode, expr, 0, size, guard_manager);
}
  ...
  guard_managert &guard_manager)
{
  rw_range_sett::get_objects_dereference(
    mode, deref, range_start, size, guard_manager);

  exprt object=deref;
  dereference(target, object, ns, value_sets, guard_manager);
  ...
  if(object.is_not_nil() && !has_subexpr(object, ID_dereference))
    get_objects_rec(mode, object, range_start, new_size, guard_manager);
}
