...
Function: string_refine_preprocesst::new_tmp_symbol
 ...
 Purpose: add a temporary symbol with the given name and type to the
...
{
  // TODO : this method should be replaced by a call to get_fresh_aux_symbol
  // once fresh symbol generation has been factored out
  auxiliary_symbolt tmp_symbol;
  std::ostringstream buf;
  buf << "string_preprocess_tmp" << (next_symbol_id++) << "$" << prefix;
...
Function: string_refine_preprocesst::new_symbol

  Inputs: a name and a type

 Outputs: a new symbol

 Purpose: add a symbol with the given name and type to the symbol table
          and returns it

\*******************************************************************/

symbol_exprt string_refine_preprocesst::new_symbol(
  const std::string &prefix, const typet &type, bool static_lifetime)
{
  symbolt tmp_symbol;
  std::ostringstream buf;
  ...
  tmp_symbol.is_state_var=static_lifetime;
  tmp_symbol.is_type=false;
  tmp_symbol.is_lvalue=true;
  tmp_symbol.mode=ID_java;
  tmp_symbol.name=name;
  tmp_symbol.type=type;
  symbol_table.add(tmp_symbol);
  ...
  const std::string &signature)
{
  ...
  symbol_exprt tmp_length=new_tmp_symbol("tmp_length", length_type);
  symbol_exprt tmp_array=new_tmp_symbol("tmp_array", data_type.subtype());
  ...
  symbol_exprt cprover_string_sym=new_tmp_symbol(
    "tmp_cprover_string", ref_type);
  ...
  bool skip_first_arg)
{
  ...
  make_string_function(
    goto_program, i_it, lhs, new_type, function_name, args, loc, new_sig);
}
