...
void character_refine_preprocesst::convert_get_directionality_int(
  conversion_input &target)
{
  convert_get_directionality_char(target);
}
  ...
  const exprt &chr, const typet &type)
{
  ...
  intervals.push_back(in_interval_expr(chr, 0x1D800, 0x1FFFF));
  ...
  const exprt &chr, const typet &type)
{
  ...
  exprt condition0=in_list_expr(chr, title_case_chars);
  exprt condition1=in_interval_expr(chr, 0x1F88, 0x1F8F);
  exprt condition2=in_interval_expr(chr, 0x1F98, 0x1F9F);
  exprt condition3=in_interval_expr(chr, 0x1FA8, 0x1FAF);
  return or_exprt(
...
{
  or_exprt res(
    expr_of_is_unicode_identifier_start(chr, type),
    or_exprt(expr_of_is_digit(chr, type),
             expr_of_is_identifier_ignorable(chr, type)));
  return res;
}
  ...
  const exprt &chr, const typet &type)
{
  ...
  exprt condition0=in_list_expr(chr, space_characters);
  exprt condition1=in_interval_expr(chr, 0x2000, 0x2006);
  exprt condition2=in_interval_expr(chr, 0x2008, 0x200A);
  exprt condition3=in_interval_expr(chr, 0x09, 0x0D);
  exprt condition4=in_interval_expr(chr, 0x1C, 0x1F);
  return or_exprt(
  ...
  const exprt &chr, const typet &type)
{
  ...
  typet char_type=array_type.subtype();
