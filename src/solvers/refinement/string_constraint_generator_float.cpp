...
exprt string_constraint_generatort::add_axioms_for_fractional_part(
  const array_string_exprt &res, const exprt &int_expr, size_t max_size)
{
    ...
    const array_string_exprt &res, const exprt &f)
{
  ...
  const array_string_exprt string_expr_with_E=fresh_string(index_type, char_type);
  ...
  const array_string_exprt exponent_string=fresh_string(index_type, char_type);
