...
{
  return "forall x in [" + from_expr(ns, identifier, expr.univ_lower_bound()) +
         ", " + from_expr(ns, identifier, expr.univ_upper_bound()) + "). " +
         from_expr(ns, identifier, expr.premise()) + " => (" + "exists y in [" +
         from_expr(ns, identifier, expr.exists_lower_bound()) + ", " +
...
static bool universal_only_in_index(const string_constraintt &expr)
{
  ...
  while(!stack.empty())
  {
    ...
    const unsigned child_index_depth =
      index_depth + (e.id() == ID_index ? 0 : 1);
  ...
  const string_constraintt &expr)
{
  ...
  for(const auto &pair : body_indices)
  {
    ...
    for(size_t j = 0; j < pair.second.size() - 1; j++)
    {
      ...
      if(result.is_false())
      {
        stream << "Indices not equal: " << from_expr(ns, "", expr)
               << ", str: " << from_expr(ns, "", pair.first) << eom;
        return false;
      }
    }
    ...
    {
      stream << "f is not linear: " << from_expr(ns, "", expr)
             << ", str: " << from_expr(ns, "", pair.first) << eom;
      return false;
    }
