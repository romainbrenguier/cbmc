...
public:
  ...
  refined_string_exprt add_axioms_for_refined_string(const exprt &expr);
#endif
...
private:
  ...
  array_string_exprt fresh_string(
  ...
  exprt add_axioms_for_is_prefix(
    ...
    const array_string_exprt &str, const exprt &offset);
  exprt add_axioms_for_is_prefix(
  ...
  exprt add_axioms_from_char(const array_string_exprt &res,
    const exprt &i);
  ...
  exprt add_axioms_for_string_of_float(
    const function_application_exprt &f);
  exprt add_axioms_for_string_of_float(
    const array_string_exprt &res, const exprt &f);

  exprt add_axioms_for_fractional_part(
    const array_string_exprt &res, const exprt &i, size_t max_size);
  ...
  exprt add_axioms_for_to_upper_case(
    const array_string_exprt &res, const array_string_exprt &expr);
  ...
  array_string_exprt add_axioms_for_value_of(const function_application_exprt &f);

  exprt add_axioms_for_code_point(
    const array_string_exprt &res, const exprt &code_point);
...
private:
  ...
  // the map contains an entry `dynamic_object` --> `char_array`.
  // replace_mapt address_map_;
};
