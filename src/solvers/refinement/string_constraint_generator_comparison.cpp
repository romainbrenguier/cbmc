  ...
  const function_application_exprt &f)
{
  ...
  const char_array_exprt s1=get_string_expr(f.arguments()[0]);
  const char_array_exprt s2=get_string_expr(f.arguments()[1]);
  ...
  const symbol_exprt qvar=fresh_univ_index("QA_equal_ignore_case", index_type);
  const exprt constr2=character_equals_ignore_case(
    s1[qvar], s2[qvar], char_a, char_A, char_Z);
  const string_constraintt a2(qvar, s1.length(), eq, constr2);
  axioms.push_back(a2);

  const symbol_exprt witness=fresh_exist_index(
