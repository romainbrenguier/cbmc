  ...
  array_poolt &pool)
{
  ...
  const or_exprt guard1(
    ...
    and_exprt(axiom_for_length_ge(s1, s2.length()), axiom_for_length_gt(s2, x)));
  const and_exprt cond1(ret_char_diff, guard1);
  const or_exprt guard2(
    and_exprt(axiom_for_length_gt(s2, s1.length()), axiom_for_has_length(s1, x)),
    and_exprt(axiom_for_length_gt(s1, s2.length()), axiom_for_has_length(s2, x)));
